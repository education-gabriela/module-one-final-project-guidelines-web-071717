class Stat
  ...
  def self.show_characters_stats(limit = 10)
    stats = Character.joins(:lines).group("characters.id").limit(limit.to_i)
  ...
  def self.show_episodes_stats(limit = 10)
    stats = Episode.valid_episodes.joins(:lines).group(:id).limit(limit.to_i)
  ...
  def self.show_locations_stats(limit = 10)
    stats = Line.joins(:location).joins(:appearance).group(:location_id).order("total_lines DESC").limit(limit.to_i)
